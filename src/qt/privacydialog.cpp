                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCRAVE ought to be enough for anybody." - Bill Gates, 2017
    ui->zCRAVEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCRAVESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCRAVE->setEnabled(false);
        ui->pushButtonMintzCRAVE->setToolTip(tr("zCRAVE is currently disabled due to maintenance."));

        ui->pushButtonSpendzCRAVE->setEnabled(false);
        ui->pushButtonSpendzCRAVE->setToolTip(tr("zCRAVE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCRAVEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCRAVE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRAVE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCRAVE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCRAVE in ") + 
...
void PrivacyDialog::on_pushButtonSpendzCRAVE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRAVE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCRAVE
        sendzCRAVE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCRAVE
    sendzCRAVE();
}

void PrivacyDialog::on_pushButtonZCraveControl_clicked()
{
    ZCraveControlDialog* zCRAVEControl = new ZCraveControlDialog(this);
    zCRAVEControl->setModel(walletModel);
    zCRAVEControl->exec();
}

void PrivacyDialog::setZCraveControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCRAVESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCRAVE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Crave Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCRAVEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCRAVE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CRAVE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCRAVE</b>";
    ...
    // use mints from zCRAVE selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCraveControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCraveControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcrave selector in case it was used
    ZCraveControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCRAVE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Crave, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCRAVE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCRAVE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCRAVE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCRAVE "));
    ui->labelzCRAVEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCRAVESyncStatus->setVisible(fShow);
}
